...
public class MutableHistogram extends MutableMetric implements MetricHistogram {
  ...
  protected void updateSnapshotMetrics(MetricsRecordBuilder metricsRecordBuilder,
    ...
    metricsRecordBuilder.addCounter(Interns.info(NUM_OPS_METRIC_NAME, desc), counter.get());
    metricsRecordBuilder.addGauge(Interns.info(MIN_METRIC_NAME, desc), histo.getMin());
    metricsRecordBuilder.addGauge(Interns.info(MAX_METRIC_NAME, desc), histo.getMax());
    metricsRecordBuilder.addGauge(Interns.info(MEAN_METRIC_NAME, desc), histo.getMean());

    long[] percentiles = histo.getQuantiles();

    metricsRecordBuilder.addGauge(Interns.info(TWENTY_FIFTH_PERCENTILE_METRIC_NAME, desc),
        percentiles[0]);
    metricsRecordBuilder.addGauge(Interns.info(MEDIAN_METRIC_NAME, desc),
        percentiles[1]);
    metricsRecordBuilder.addGauge(Interns.info(SEVENTY_FIFTH_PERCENTILE_METRIC_NAME, desc),
        percentiles[2]);
    metricsRecordBuilder.addGauge(Interns.info(NINETIETH_PERCENTILE_METRIC_NAME, desc),
        percentiles[3]);
    metricsRecordBuilder.addGauge(Interns.info(NINETY_FIFTH_PERCENTILE_METRIC_NAME, desc),
        percentiles[4]);
    metricsRecordBuilder.addGauge(Interns.info(NINETY_EIGHTH_PERCENTILE_METRIC_NAME, desc),
        percentiles[5]);
    metricsRecordBuilder.addGauge(Interns.info(NINETY_NINETH_PERCENTILE_METRIC_NAME, desc),
        percentiles[6]);
    metricsRecordBuilder.addGauge(
        Interns.info(NINETY_NINE_POINT_NINETH_PERCENTILE_METRIC_NAME, desc),
        percentiles[7]);
  }
}
