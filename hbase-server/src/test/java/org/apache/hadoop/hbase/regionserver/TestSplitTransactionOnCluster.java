 ...
 /**
  ...
  public void testTableExistsIfTheSpecifiedTableRegionIsSplitParent() throws Exception {
    ...
    try {
      ...
      // Turn off the meta scanner so it don't remove parent on us.
      cluster.getMaster().setCatalogJanitorEnabled(false);
      boolean tableExists = MetaReader.tableExists(regionServer.getCatalogTracker(HRegionInfo.DEFAULT_REPLICA_ID),
          tableName);
      ...
      tableExists = MetaReader.tableExists(regionServer.getCatalogTracker(HRegionInfo.DEFAULT_REPLICA_ID),
          tableName);
      assertEquals("The specified table should present.", true, tableExists);
    } finally {
      if (regions != null) {
  ...
  public void testSplitWithRegionReplicas() throws Exception {
    ...
    try {
      ...
      // Turn off the meta scanner so it don't remove parent on us.
      cluster.getMaster().setCatalogJanitorEnabled(false);
      boolean tableExists = MetaReader.tableExists(new CatalogTracker(TESTING_UTIL.getConfiguration()),
          tableName);
      ...
      do {
        ...
        Thread.sleep(1000);
          ...
          || newRegions.size() != 4);
      tableExists = MetaReader.tableExists(new CatalogTracker(TESTING_UTIL.getConfiguration()),
          tableName);
      assertEquals("The specified table should be present.", true, tableExists);
