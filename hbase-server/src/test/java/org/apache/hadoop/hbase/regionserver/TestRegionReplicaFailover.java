...
public class TestRegionReplicaFailover {
  ...
  public void before() throws IOException {
    htd = HTU.createTableDescriptor(name.getMethodName());
  ...
  public void after() throws IOException {
    HTU.deleteTableIfAny(TableName.valueOf(name.getMethodName()));
  }
  ...
  public void testSecondaryRegionWithEmptyRegion() throws IOException {
    ...
    HTableInterface table = connection.getTable(htd.getTableName());
   ...
   */
  @Test (timeout = 120000)
  public void testPrimaryRegionKill() throws IOException {
    HConnection connection = HConnectionManager.createConnection(HTU.getConfiguration());
    HTableInterface table = connection.getTable(htd.getTableName());
    ...
    HTU.verifyNumericRows(table, fam, 0, 1000, 0);
    HTU.verifyNumericRows(table, fam, 0, 1000, 1);
   ...
   */
  @Test (timeout = 120000)
  public void testSecondaryRegionKill() throws IOException {
    HConnection connection = HConnectionManager.createConnection(HTU.getConfiguration());
    HTableInterface table = connection.getTable(htd.getTableName());
  ...
  public void testSecondaryRegionKillWhilePrimaryIsAcceptingWrites() throws Exception {
    final HConnection connection = HConnectionManager.createConnection(HTU.getConfiguration());
    final HTableInterface table = connection.getTable(htd.getTableName());
    final HBaseAdmin admin = new HBaseAdmin(HTU.getConfiguration());
    ...
    HTU.verifyNumericRows(table, fam, 0, key.get(), 1);
    HTU.verifyNumericRows(table, fam, 0, key.get(), 2);
    table.close();
    admin.close();
    connection.close();
  ...
  public void testLotsOfRegionReplicas() throws IOException {
    ...
    String tableName = name.getMethodName() + "2";
