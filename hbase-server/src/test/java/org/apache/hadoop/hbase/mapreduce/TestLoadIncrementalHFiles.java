...
public class TestLoadIncrementalHFiles {
  ...
  private void runTest(String testName, HTableDescriptor htd, BloomType bloomType,
      boolean preCreateTable, byte[][] tableSplitKeys, byte[][][] hfileRanges, boolean copyFiles)
          throws Exception {
    ...
    for (boolean managed : new boolean[] { true, false }) {
      ...
      for (byte[][] range : hfileRanges) {
        ...
        HFileTestUtil.createHFile(util.getConfiguration(), fs, path, FAMILY, QUALIFIER, from, to,
            1000);
        if (copyFiles) list.add(path);
      }
