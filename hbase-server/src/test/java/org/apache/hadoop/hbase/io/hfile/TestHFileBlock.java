...
public class TestHFileBlock {
        ...
        if (algo == GZ) {
          ...
          try {
            b = hbr.readBlockData(0, wrongCompressedSize
                + HConstants.HFILEBLOCK_HEADER_SIZE, -1, pread, false);
    ...
    for (Compression.Algorithm algo : COMPRESSION_ALGORITHMS) {
      for (boolean pread : new boolean[] { false, true }) {
        for (DataBlockEncoding encoding : DataBlockEncoding.values()) {
          ...
          int pos = 0;
          for (int blockId = 0; blockId < numBlocks; ++blockId) {
            blockFromHFile = hbr.readBlockData(pos, -1, -1, pread, false);
        ...
        for (boolean cacheOnWrite : BOOLEAN_VALUES) {
          ...
          for (int i = 0; i < NUM_TEST_BLOCKS; ++i) {
            ...
            HFileBlock b2 = hbr.readBlockData(curOffset,
                b.getOnDiskSizeWithHeader(), -1, pread, false);
            b2.sanityCheck();
  ...
  public void testConcurrentReading() throws Exception {
	...
	// HADOOP (eg. HADOOP-7844) fix the issue or a good workaround is identified.
