...
public class ReplicationSink {
  ...
  private volatile HConnection sharedHtableCon;
  ...
  protected void batch(TableName tableName, Collection<List<Row>> allRows) throws IOException {
    ...
    try {
      ...
      HConnection connection = this.sharedHtableCon;
      if (connection == null) {
        synchronized (sharedHtableConLock) {
          connection = this.sharedHtableCon;
          if (connection == null) {
            connection = this.sharedHtableCon = HConnectionManager.createConnection(this.conf);
          }
        }
      }
