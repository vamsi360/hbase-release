...
public class HFileCleaner extends CleanerChore<BaseHFileCleanerDelegate> {
  ...
  public HFileCleaner(final int period, final Stoppable stopper, Configuration conf, FileSystem fs,
    ...
    largeFileQueue = new LinkedBlockingQueue<HFileCleaner.HFileDeleteTask>(largeQueueInitSize);
    smallFileQueue = new LinkedBlockingQueue<HFileCleaner.HFileDeleteTask>(smallQueueInitSize);
  ...
  private void countDeletedFiles(boolean isLargeFile) {
    if (isLargeFile) {
