...
public final class TableName implements Comparable<TableName> {
  ...
  public static TableName valueOf(byte[] fullName) throws IllegalArgumentException{
    ...
    final int namespaceDelimIndex = Bytes.toString(fullName).lastIndexOf(NAMESPACE_DELIM);
    ...
    if (namespaceDelimIndex < 0) {
      ...
      final String fullNameStr = Bytes.toString(fullName);
      // indexOf is by character, not byte (consider multi-byte characters)
      String ns = fullNameStr.substring(0, namespaceDelimIndex);
      String qualifier = fullNameStr.substring(namespaceDelimIndex + 1);
      return createTableNameIfNecessary(
