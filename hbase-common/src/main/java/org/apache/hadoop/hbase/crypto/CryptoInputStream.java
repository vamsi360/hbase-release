 ...
 */
package org.apache.hadoop.hbase.crypto;
...
import org.apache.hadoop.hbase.security.BufferRecycler;
...
public class CryptoInputStream extends FilterInputStream implements 
  ...
  public CryptoInputStream(InputStream in, CryptoCodec codec,
      int bufferSize, byte[] key, byte[] iv, DirectBufferPool pool) throws IOException {
    this(in, codec, bufferSize, key, iv, 
        in != null ? CryptoStreamUtils.getInputStreamOffset(in) : 0, pool);
  }
  
  public CryptoInputStream(InputStream in, CryptoCodec codec,
      int bufferSize, byte[] key, byte[] iv, long streamOffset, DirectBufferPool pool) throws IOException {
    super(in);
    ...
    inBuffer = this.pool.getBuffer(this.bufferSize);
    outBuffer = this.pool.getBuffer(this.bufferSize);
    decryptor = getDecryptor();
    resetStreamOffset(streamOffset);
  }
  
  public CryptoInputStream(InputStream in, CryptoCodec codec,
      byte[] key, byte[] iv, DirectBufferPool pool) throws IOException {
    this(in, codec, CryptoStreamUtils.getBufferSize(codec.getConf()), key, iv, pool);
  }
  ...
  private byte[] getTmpBuf() {
    if (tmpBuf == null) {
      tmpBuf = BufferRecycler.recycler().allocate(bufferSize);
    }
    return tmpBuf;
  }
  ...
  private ByteBuffer getBuffer() {
    ByteBuffer buffer = this.pool.getBuffer(this.bufferSize);
    return buffer;
  }
  ...
  private void freeBuffers() {
    this.pool.returnBuffer(inBuffer);
    inBuffer = null;
    this.pool.returnBuffer(outBuffer);
    outBuffer = null;
    if (tmpBuf != null) {
      BufferRecycler.recycler().release(tmpBuf);
      tmpBuf = null;
    }
  }
